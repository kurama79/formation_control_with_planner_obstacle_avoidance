...
class FloatPair:
	...
	def __gt__(self, other):
		...
		# Returns true if the first pair of scalar values is greater than the second paor of scalar values.

		return not (self <= other) 

	def __ge__(self, other):

		# Returns true if the first pair of scalar values is greater than or equal to the second paor of scalar values.
...
class KdTree:
	...
	def build_obstacle_tree(self):
		...
		# Build an obstacle k-D tree.

		obstacles = self.simulator_.obstacles_[0]
		# obstacles = list(self.simulator_.obstacles_[0])
	...
	def compute_agent_neighbors(self, agent, rangeSq):
		...
		Args:
			agent (Agent): The agent for chich agent neighbors are to be computed.
	...
	def build_agent_tree_recursive(self, begin, end, node):
		...
		Args:
			begin (int): The beginning agent k-D tree node index.
	...
	def build_obstacle_treeRecursive(self, obstacles):
		...
		for j in range(len(obstacles)):
			...
			if j1LeftOfI >= -rvo_math.EPSILON and j2LeftOfI >= -rvo_math.EPSILON:
				...
				newObstacle.id_ = len(self.simulator_.obstacles_[0])

				self.simulator_.obstacles_[0].append(newObstacle)
	...
	def query_agent_tree_recursive(self, agent, rangeSq, node):
		...
		'''
		Recurisve method for computing the agent neighbors of the specified agent.
